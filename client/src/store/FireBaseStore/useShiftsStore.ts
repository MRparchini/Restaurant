...
import { addDoc, collection, deleteDoc, doc, getDocs, query, where, writeBatch, updateDoc } from 'firebase/firestore';
...
export const useShiftStore = create<ShiftState>((set, get) => ({
  ...
  fetchShifts: async (_startDate, _endDate) => {
    ...
    try {
      ...
      const filteredShifts: Shift[] = snapshot.docs.map((d: any) => ({ id: d.id, ...(d.data() as Omit<Shift, 'id'>) }));
  ...
  addShift: async (shift) => {
    set({ loading: true, error: null });
    try {
      const payload = { ...shift, created_at: new Date().toISOString() } as Omit<Shift, 'id'>;
      const ref = await addDoc(collection(db, 'shifts'), payload);
      set((state) => ({
        shifts: [...state.shifts, { id: ref.id, ...payload } as Shift],
        loading: false
      }));
    } catch (err) {
      set({
  ...
  addWeeklySchedule: async (schedule: WeeklySchedule) => {
    ...
    try {
      ...
      const qDelete = query(
        ...
        where('shift_date', '>=', firstDate),
        where('shift_date', '<=', lastDate)
      );
      const toDeleteSnap = await getDocs(qDelete);
      const batch = writeBatch(db);
      toDeleteSnap.forEach((docSnap: any) => batch.delete(doc(db, 'shifts', docSnap.id)));
